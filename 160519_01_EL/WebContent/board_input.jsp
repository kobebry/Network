...
<form action="write.do" method="post" name="board_form">
	<table border="1">
		...
		<tr>
			<td colspan="3">
				<input type="file" name="pds_link" value='<c:if test="${BDTO ne null}">${BDTO.pds_link}</c:if>'>
			</td>
		</tr>
		<tr>
			<th colspan="2">본문 내용</th>
			<td colspan="3"><textarea rows="10" cols="20" name="contents"><c:if test="${BDTO ne null}">${BDTO.contents}</c:if></textarea></td>
		</tr>
	</table>
	<c:choose>
		<c:when test="${BDTO ne null}">
		<p><input type="button" value="수정" onclick="w_accept('mod')"></p>
		<p><input type="button" value="취소" onclick="w_list()"></p>
		   <input type="hidden" name="p_bid" value="${BDTO.board_id}">
		
		</c:when>
		<c:otherwise>
		<p><input type="button" value="작성" onclick="w_accept('add')"></p>		
		</c:otherwise>
	</c:choose>
